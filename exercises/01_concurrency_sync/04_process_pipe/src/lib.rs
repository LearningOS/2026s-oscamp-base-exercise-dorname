...
pub fn run_command(program: &str, args: &[&str]) -> String {
    ...
    let cmd = Command::new(program).args(args).output().unwrap();
...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    // 创建 cat 命令，并将标准输入（stdin）和标准输出（stdout）设置为管道模式
    // Spawn process 启动（创建）子进程
    // Write input to child process stdin 将输入数据写入子进程的标准输入（stdin）
    // Drop stdin to close pipe (otherwise cat won't exit) 释放（drop）stdin 以关闭管道（否则 cat 不会退出）
    // Read output from child process stdout 从子进程的标准输出（stdout）读取输出数据
