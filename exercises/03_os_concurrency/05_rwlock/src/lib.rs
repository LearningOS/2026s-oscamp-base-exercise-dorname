...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        loop {
            ...
            // If a writer is holding or waiting, spin and continue
            if state & (WRITER_HOLDING | WRITER_WAITING) != 0 {
                std::hint::spin_loop();
                continue;
            }
            // If the reader count is already at the maximum, spin and continue
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            let state = self.state.load(Ordering::Acquire);
            // If any readers or a writer is holding, spin and continue
            if state & READER_MASK != 0 || state & WRITER_HOLDING != 0 {
                std::hint::spin_loop();
                continue;
            }
            // Try to take the lock
            match self.state.compare_exchange(state, WRITER_HOLDING, Ordering::AcqRel, Ordering::Acquire) {
